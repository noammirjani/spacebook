(function () {
	...
	/**
	 * Handles the response, error, and json parsing for fetch requests.
	 * @function
	 */
	const fetchHandlers = (function () {
		...
		function handleErrorComments(error) {
			...
			if (error.message.includes("Unexpected token")) {
				...
				selectors.commentsErrorMsg.innerHTML =
		...
		function initCommentFetch(dateStr) {
			if (!date.valid(dateStr)) fetchHandlers.handleErrorLoad("date pattern is not valid");
		}
	...
	const bookPage = (function () {
		...
		function failedLoad(error, scrollMode) {
			...
			if (!scrollMode) fetchHandlers.handleErrorLoad(error);
			else fetchHandlers.handleErrorScroll(error);
		}
	...
	const feedCreate = (function () {
		...
		function createCardGrid(data) {
			...
			for (const imgData of Object.values(data).reverse()) {
				...
				if (imgData.media_type == "image") col1.appendChild(feedCreate.createImage(imgData.url, imgData.date));
	...
	const comments = (function () {
		 ...
		 * @param {object} bodyData - The data to include in the body of the request, in JSON format
		 */
